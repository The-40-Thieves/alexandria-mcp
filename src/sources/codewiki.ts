...
function collectFrames(
  node: unknown,
  out: Array<{ rpcId: string; payload: unknown }>,
): void {
...
function extractPayload(responseText: string, rpcId: string): unknown {
  ...
  if (!match) {
    ...
    throw new Error(`CodeWiki RPC ${rpcId} not found in response (available: ${available || 'none'})`);
  }
...
function normalizeRepo(input: string): {
  repoPath: string; repoUrl: string; sourcePath: string;
} {
...
export async function codewikiSearch(query: string, limit = 10): Promise<LibraryResult[]> {
  ...
  return (rows)
    ...
    .map(item => {
      ...
      const url = Array.isArray(item[3]) && typeof (item[3] as unknown[])[1] === 'string'
        ? (item[3] as string[])[1]
        : null;
...
export async function codewikiRead(repoInput: string): Promise<{
  ...
  const sections = sectionsRaw
    ...
    .map(item => {
      ...
      const markdown = typeof item[5] === 'string' ? item[5]
        : typeof item[4] === 'string' ? item[4]
...
register('codewiki', {
  description: 'Google Code Wiki — AI-generated continuously updated docs for any GitHub repo. Search indexed repos or fetch architecture overviews, API references, and class diagrams by passing "{owner}/{repo}". No auth required.',
  supportsIngest: true,
