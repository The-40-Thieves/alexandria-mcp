...
export async function ostiSearch(query: string, limit = 10): Promise<LibraryResult[]> {
  ...
  return (records || []).map(r => ({
    ...
    title: r.title,
    authors: (r.authors || []).map(a => `${a.first_name || ''} ${a.last_name || ''}`.trim()).filter(Boolean),
    year: r.publication_date ? parseInt(r.publication_date.substring(0, 4), 10) : undefined,
    subjects: r.subject_areas ? r.subject_areas.split(';').map(s => s.trim()).filter(Boolean) : [],
    language: r.language,
...
register('osti', {
  description: 'DOE OSTI — Department of Energy research: nuclear, energy, physics, materials science. No API key required.',
  supportsIngest: true,
  search: ostiSearch,
  async read(id) {
