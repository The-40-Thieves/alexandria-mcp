...
import { recordFullTextRead, resetFullTextWindow, troveRead } from '../trove.js';
...
test('troveRead', async (t) => {
  await t.test('without TROVE_API_KEY, returns the pre-Stage-2 metadata-only shape with no network calls', async () => {
  ...
  await t.test('with a key, follows a fulltext newspaper link and returns article text', async () => {
    const saved = process.env.TROVE_API_KEY;
    ...
    globalThis.fetch = (async (url: string | URL | Request) => {
      const u = String(url);
      if (u.includes('/work/151678302')) return new Response(fixture('trove-work'), { status: 200 });
  ...
  await t.test('with a key, falls back to metadata-only when the work has no fulltext link', async () => {
    const saved = process.env.TROVE_API_KEY;
    process.env.TROVE_API_KEY = 'test-key';
