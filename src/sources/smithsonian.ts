...
function key(): string {
  ...
  if (!k) throw new Error('SMITHSONIAN_API_KEY is not set. Register free at https://api.data.gov/signup/');
  return k;
}
...
export async function smithsonianRead(id: string): Promise<{
  ...
  const data = await fetchJSON<SIRecord>(
    `${BASE}/content/${id}?api_key=${key()}`
  );
  ...
  const text = [...notes, ...desc].map(n => `${n.label}: ${n.content}`).join('\n') ||
...
register('smithsonian', {
  description: 'Smithsonian Institution — 14M+ records across all museums: American history, air & space, natural history, art, military artifacts. Requires SMITHSONIAN_API_KEY (free at api.data.gov).',
  supportsIngest: true,
