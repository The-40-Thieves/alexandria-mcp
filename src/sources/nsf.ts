...
export async function nsfSearch(query: string, limit = 10): Promise<LibraryResult[]> {
  ...
  return (data.response?.award || []).map(a => ({
    ...
    subjects: a.keywords ? a.keywords.split(',').map(s => s.trim()).filter(Boolean) : [],
...
export async function nsfRead(id: string): Promise<{
  ...
  const data = await fetchJSON<NSFResponse>(
    `${BASE}?id=${id}&printFields=${PRINT_FIELDS}`
  );
...
register('nsf', {
  description: 'NSF Awards — National Science Foundation research grants and abstracts across all scientific disciplines.',
  supportsIngest: true,
  search: nsfSearch,
  async read(id) {
