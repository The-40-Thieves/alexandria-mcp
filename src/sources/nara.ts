...
interface NARARecord {
  ...
  description?: {
    item?: {
      ...
      generalNote?: string;
      dateRange?: { inclusiveDates?: string };
    };
...
function getNote(rec: NARARecord): string {
  return (
    rec.description?.item?.scopeAndContentNote ||
...
function getTitle(rec: NARARecord): string {
  return (
    rec.title ||
    rec.description?.series?.title ||
    rec.description?.recordGroup?.title ||
    String(rec.naId || '')
  );
}
...
export async function naraRead(id: string): Promise<{
  ...
  const text =
    [note, transcriptions].filter(Boolean).join('\n\n') ||
...
register('nara', {
  description:
    'US National Archives (NARA) — 32M+ descriptions of historical government records: war diaries, presidential papers, military records, federal agencies. No API key required.',
  supportsIngest: true,
  search: naraSearch,
  async read(id) {
